...
public final class Interpreter {
    ...
    private func registerBridgeTypesAsGlobals() {
        ...
        // Register container view types that accept trailing closures.
        // These aren't in the BridgeRegistry — they need VM access to invoke closures.
        #if canImport(SwiftUI)
        registerContainerGlobals()
        #endif
    }

    #if canImport(SwiftUI)
    /// Register container view types (VStack, HStack, etc.) as VM globals.
    /// These accept a trailing closure which is invoked to collect child views.
    private func registerContainerGlobals() {
        let containerTypes = ["VStack", "HStack", "ZStack", "ScrollView", "Group", "List", "NavigationStack"]

        for typeName in containerTypes {
            let vm = self.vm
            let fn = NativeFunctionRef(name: typeName, arity: -1) { [weak vm] args in
                guard let vm else { return .nil_ }
                // Separate numeric args (spacing) from closure args
                ...
                // Invoke the trailing closure to get child view(s)
                ...
                // Build the container — use ViewBox wrapper to avoid Sendable issues
        ...
        // Button — special: takes title or label closure + action closure
        let buttonVM = self.vm
        let buttonFn = NativeFunctionRef(name: "Button", arity: -1) { [weak buttonVM] args in
            guard let vm = buttonVM else { return .nil_ }
            ...
            // If we have title + closure: Button("title") { action }
            // If we have two closures: Button { action } label: { label }
            let action: () -> Void = {
        ...
        vm.environment.setGlobal("Button", value: .nativeFunction(buttonFn))
    }
    #endif
